...
class AuthRepositoryImpl(
    ...
    override suspend fun login(email: String, password: String): Result<String> {
        return safeApiCall { authApi.login(email, password).token }
    }

    override suspend fun register(name: String, email: String, password: String): Result<Unit> {
        return safeApiCall { authApi.register(name, email, password) }
    }

    override suspend fun validateCode(email: String, code: String): Result<String> {
        return safeApiCall { authApi.validateCode(email = email, code = code).token }
    }

    override suspend fun resendVerificationCode(email: String): Result<Unit> {
        return safeApiCall { authApi.resendVerificationCode(email) }
    }

    override suspend fun loginWithGoogle(idToken: String): Result<String> {
        return safeApiCall { authApi.loginWithGoogle(idToken).token }
    }

    override suspend fun logout(): Result<Boolean> {
        return safeApiCall { authApi.logout() }
    }
}
