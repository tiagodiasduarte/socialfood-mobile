...
class FavouriteRestaurantsRepositoryImplTest {
    ...
    @Test
    fun `given api throws when markFavourite is called then still returns Success with entity left PENDING_ADD`() = runTest {
        // Given
    ...
    @Test
    fun `given api throws when unmarkFavourite is called then still returns Success with entity left PENDING_REMOVE`() = runTest {
        // Given
        val (repo, dao, _) = createRepository(api = FakeFavouriteRestaurantsApi(shouldThrow = true))
        dao.upsert(fakeRestaurant.toFavouriteRestaurantEntityForTest(FavouriteSyncState.SYNCED))
    ...
    @Test
    fun `given cached favourites when getFavouritesPaged is called then reads from DAO only and never calls the API`() = runTest {
        // Given
        val (repo, dao, _) = createRepository(api = FakeFavouriteRestaurantsApi(shouldThrow = true))
        dao.upsert(fakeRestaurant.toFavouriteRestaurantEntityForTest(FavouriteSyncState.SYNCED))
        ...
        assertEquals(fakeRestaurant.id, result.data.favourites.first().restaurant.id)
    }

    // syncFavourites

    @Test
    fun `given changes available when syncFavourites is called then applies them and advances checkpoint`() = runTest {
        // Given
    ...
    @Test
    fun `given DAO write throws when syncFavourites is called then does not advance the checkpoint`() = runTest {
        // Given
    ...
    @Test
    fun `given last sync attempt was recent when syncFavourites is called then returns early without calling the API`() = runTest {
        // Given
