...
class AuthApiImpl(private val client: HttpClient) : AuthApi {
    ...
    override suspend fun loginWithGoogle(idToken: String): LoginResponse = client.post("auth/google") {
        contentType(ContentType.Application.Json)
        setBody(GoogleLoginRequest(idToken = idToken))
    }.body()

    override suspend fun validateCode(email: String, code: String): ValidateCodeResponse = client.post("auth/verify") {
        contentType(ContentType.Application.Json)
        setBody(ValidateCodeRequest(email = email, code = code))
    }.body()

    override suspend fun resendVerificationCode(email: String): Unit = client.post("auth/send-verification") {
        contentType(ContentType.Application.Json)
        setBody(ResendVerificationCodeRequest(email = email))
    }.body()

    override suspend fun logout(): Boolean = client.post("auth/logout") {
        contentType(ContentType.Application.Json)
    }.body()
}
