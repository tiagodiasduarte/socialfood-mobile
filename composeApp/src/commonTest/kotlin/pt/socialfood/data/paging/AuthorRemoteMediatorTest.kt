...
class AuthorRemoteMediatorTest {
    ...
    private val emptyState = PagingState<Int, AuthorEntity>(
        pages = emptyList(),
        anchorPosition = null,
        config = config,
    ...
    @Test
    fun `given REFRESH succeeds when load completes then old cached rows and the remote key are replaced not merged`() = runTest {
        // Given
        val authorDao = FakeAuthorDao()
        val authorRemoteKeyDao = FakeAuthorRemoteKeyDao()
