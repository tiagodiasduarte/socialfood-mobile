...
class DeleteHomeSectionUseCaseImplTest {
    @Test
    fun `given repository succeeds when invoked then returns Success and forwards the id`() = runTest {
        // Given
        val id = Random.nextString()
        val repository = FakeHomeRepository(deleteResult = Result.Success(true))
        val useCase = DeleteHomeSectionUseCaseImpl(repository)
