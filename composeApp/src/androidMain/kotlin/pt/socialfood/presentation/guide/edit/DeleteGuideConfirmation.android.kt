...
actual fun rememberDeleteGuideConfirmationLauncher(onConfirm: () -> Unit): () -> Unit {
    ...
    if (showDialog) {
        AlertDialog(
            ...
            title = { Text(stringResource(Res.string.edit_guide_delete_confirmation_title)) },
            text = { Text(stringResource(Res.string.edit_guide_delete_confirmation_message)) },
            confirmButton = {
                TextButton(
                    onClick = {
                        showDialog = false
                    ...
                    Text(stringResource(Res.string.edit_guide_delete_confirmation_confirm))
                }
            },
            dismissButton = {
                TextButton(onClick = { showDialog = false }) {
                    Text(stringResource(Res.string.edit_guide_delete_confirmation_cancel))
                }
            },
        )
    }
