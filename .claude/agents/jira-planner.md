...
First, write a short description to `.plans/<TICKET-ID>-jira.md`:
...
Then source `scripts/jira.sh` and call `jira_update_description <TICKET-ID>`:

```bash
source ./scripts/jira.sh
jira_update_description <TICKET-ID>
```

This pushes `.plans/<TICKET-ID>-jira.md` to the Jira ticket description (the full plan stays local in `.plans/<TICKET-ID>.md`). Report the ticket URL when done.