...
class EditProfileViewModelTest {
    ...
    fun `given the S3 upload fails when save is called then the user photo is not updated`() =
        runTestWithMainDispatcher {
            // Given
            val uploadPhoto = FakeUploadPhotoUseCase(Result.Failure(ApiError.Network(Exception("test error"))))
    ...
    fun `given updateUser fails when save is called then saveError is set`() = runTestWithMainDispatcher {
        // Given
        val updateUser = FakeUpdateUserUseCase(Result.Failure(ApiError.Network(Exception("test error"))))
